...
if (AntiLink) {
    ...
    Miku.sendMessage(from, {text:`\`\`\`『 منع الروابط 』\`\`\`\n\n@${kice.split("@")[0]}تم طرده لإرسال رابط في الجروب`, contextInfo:{mentionedJid:[kice]}}, {quoted:m})
    } else {
    }
    }
  ...
  Miku.sendMessage(from, {text:`\`\`\`『 ''wa.me' منع روابط! 』\`\`\`\n\n@${kice.split("@")[0]}تم طرده لإرسال رابط في الجروب!`, contextInfo:{mentionedJid:[kice]}}, {quoted:m})
  } else {
  }
  if (antiWame)
...
const ftroli = {
  ...
  const jumlahUser = pendaftar.length
    ...
    const menulist = `
    ELGAZAR ${pushname} 👋.انا العزيز ${global.BotName}, بوت تم تطويري بواسطة: عبدالله محمد للانتقال باستخدام واتساب إلى المستوى التالي.
...
switch(command) {
    ...
    case 'sc': case 'التبرع': case 'التبرع': {
...
case 'support': case 'الدعم':
    ...
    reply(`*جروب الدعم:*https://chat.whatsapp.com/BouqFdZoBXwA84MYpOJGk8 `)
    break
...
case 'احسب': case 'حساب': case 'calculate':{
   if (isBan) return reply(mess.banned)	 			
if (isBanChat) return reply(mess.bangc)
...
reply(`\`\`\`「 *حساب* 」\`\`\`\n\n*السؤال :* ${qsd}\n*الاجابه :* ${mathjs.evaluate(qsd.replace(/×/g, "*").replace(/x/g, "*").replace(/÷/g, "/"))}`)
}
}
break
...
*الرابط :* ${anu.url}`,
 ...
 case 'getmusic': case 'getvideo': case 'yt': case 'youtube': case 'بحث': case 'ytmp4': case 'ytmusic': {
    if (isBan) return reply(mess.banned)	 			
 if (isBanChat) return reply(mess.bangc)
 if (!args[0]) return reply(mess.nolink)
 try {
 ...
 *TITLE:* ${data.title}\n*QUALITY:* ${data.medias[0].quality}\n*SIZE:* ${data.medias[0].formattedSize}\n*DURATION* ${data.duration}\n*LINK:* ${data.url}\n\n*${BotName}*`
   ...
   Miku.sendMessage(m.chat, { video: { url: data.medias[0].url }, jpegThumbnail:buf, caption: `*اللقب:* ${data.title}\n*الجوده:* ${data.medias[0].quality}\n*الحجم:* ${data.medias[0].formattedSize}` }, { quoted: m })  
                 }).catch((err) => {
                     reply(mess.reply)
                 })
             }
...
if ((quoted.msg || quoted).seconds > 20) return reply('الحد الأقصى المسموح به هو 10 ثواني!')
let media = await quoted.download()
...
if (!text) return reply(`Send/رد علي صوره واكتب الامر ${prefix + command} *text*`)
if (text.includes('|')) return reply(`رد علي صوره واكتب الامر ${prefix + command} *text*`)
if (!/image/.test(mime)) return reply(`رد علي صوره واكتب الامر ${prefix + command} *text*`)
reply(mess.wait)
...
case 'استيكر': case 'sticker': case 's': case 'ملصق': case 'س': {
 ...
 if ((quoted.msg || quoted).seconds > 20) return reply('حد اقصي10ثواني!')
 let media = await quoted.download()
...
case 'ويكيديا':
    if (isBan) return reply(mess.banned)
    if (isBanChat) return reply(mess.bangc)
if (args.length < 1) return reply('ما اللذي تبحث عنه?? ')
...
"ما هو أكثر شيء تضيع وقت فراغك به؟", 
...
"ما هو أكثر شيء يمكنه أن يخطف النوم من عينك؟",
  "هواية او تجربة كان ودك تستمر و تركتها ؟"
                   ...
                   Miku.sendMessage(from, { image: buffer, caption: '*〖قل الصراحه〗*\n\n'+ Mikudareww }, {quoted:m})
                   break
...
" لو خيروك بين العيش بجانب الجبال والحدائق والأشجار أو العيش بجانب البحر."
                           ...
                           Miku.sendMessage(from, { image: buffer, caption: '*〖لو خيروك〗*\n'+ mikutruthww }, {quoted:m})
                           break
